### Program implementing Neural Network Language Model (NNLM) using an RNN

## Features:
# 1. RNN allows processing of input sequence of different lengths / num of words (though, for implementation, we need to fix on a max seq len)
# 2. target word sequence is one right shifted version of the input word sequence

## Todos / questions:
# 1. [solved] Do we need additional linear layer(s) after the RNN output? [fix: Yes, since we need to transform h_dim of output to n_classes]
# 2. [solved] add function in RNN to generate text given a single word or a sequence of words as input [fix: no need to add another function, just add a for loop over RNN forward prop with a input seq_len=1 ]
# 3. [solves] cross entropy loss for a batch of sequences: flattening the score vector from [seq_len, batch_size, n_classes] to [seq_len * batch_size, n_classes] gives wrong results. [fix: The documentation of CrossEntropyLoss says it can accept [batch_size, n_classes, seq_len]. Trying that works out. But why didn't the flattening work out? How does CrossEntropyLoss operate on a batch of multi-dimensional data versus on a batch of 1-d vectors ? ]
...
class RNNLM(nn.Module):
    def __init__(self, n_classes, embed_dim, h_dim):
        ...
        self.rnn = nn.RNN(embed_dim, h_dim)
        self.fc = nn.Linear(h_dim, n_classes)

    # function to forward prop a sequence of words
    def forward(self, inputs, h0): # inputs.shape: [batch_size, seq_len]; h0.shape: [1, batch_size, h_dim]
        x = self.embedding(inputs) # x.shape: [batch_size, seq_len, embed_dim]
        x = x.transpose(0, 1) # x.shape: [seq_len, batch_size, embed_dim]
        out, hn = self.rnn(x, h0) # out.shape: [seq_len, batch_size, h_dim]; hn.shape: [1, batch_size, h_dim]
...
if __name__ == '__main__':
    ...
    # instantiate model
    model = RNNLM(vocab_size, embed_dim, h_dim)
    ...
    for ep in range(num_epochs):
        ...
        # calculate loss
        h0 = torch.zeros(1, batch_size, h_dim) # starting hidden state of RNN
    ...
    for i in range(seq_len, len(word_list)):
        ...
        h0 = torch.zeros(1, 1, h_dim) # starting hidden state of RNN
    ...
    for i in range(seq_len, len(word_list)):
        ...
        input_seq = torch.LongTensor(input_seq_idx)
        h0 = torch.zeros(1, 1, h_dim) # starting hidden state of RNN
        out, hn = model(input_seq.unsqueeze(0), h0) # out.shape: [1, n_classes, seq_len]; hn.shape: [1, 1, h_dim]
