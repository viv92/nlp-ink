### Program implementing Neural Network Language Model (NNLM) using an bi-LSTM

## Features:
# 1. bi-LSTM allows processing of input sequence of different lengths / num of words (though, for implementation, we need to fix on a max seq len)
...
class LSTMLM(nn.Module):
    def __init__(self, n_classes, embed_dim, h_dim):
        ...
        self.lstm = nn.LSTM(embed_dim, h_dim, bidirectional=True)
        self.fc = nn.Linear(h_dim*2, n_classes)

    # function to forward prop a sequence of words
    def forward(self, inputs, h0, c0): # inputs.shape: [batch_size, seq_len]; h0.shape: [2, batch_size, h_dim]
        x = self.embedding(inputs) # x.shape: [batch_size, seq_len, embed_dim]
        x = x.transpose(0, 1) # x.shape: [seq_len, batch_size, embed_dim]
        out, (hn,cn) = self.lstm(x, (h0,c0)) # out.shape: [seq_len, batch_size, 2*h_dim]; hn.shape: [2, batch_size, h_dim]
...
def calculate_loss(model, inputs, targets, h0, c0):
    scores, hn, cn = model(inputs, h0, c0) # outputs.shape: [batch_size, n_classes, seq_len]
...
if __name__ == '__main__':
    ...
    # instantiate model
    model = LSTMLM(vocab_size, embed_dim, h_dim)
    ...
    for ep in range(num_epochs):
        ...
        # calculate loss
        h0 = torch.zeros(2, batch_size, h_dim) # starting hidden state of bi-LSTM
        c0 = torch.zeros(2, batch_size, h_dim) # starting cell state of bi-LSTM
        loss = calculate_loss(model, inputs, targets, h0, c0)
    ...
    for i in range(seq_len, len(word_list)):
        ...
        h0 = torch.zeros(2, 1, h_dim) # starting hidden state of bi-LSTM
        c0 = torch.zeros(2, 1, h_dim) # starting cell state of bi-LSTM
        out, hn, cn = model(input_seq.unsqueeze(0), h0, c0) # out.shape: [1, n_classes, seq_len]
        out = out.squeeze(0)
    ...
    for i in range(seq_len, len(word_list)):
        ...
        h0 = torch.zeros(2, 1, h_dim) # starting hidden state of bi-LSTM
        c0 = torch.zeros(2, 1, h_dim) # starting cell state of bi-LSTM
        out, hn, cn = model(input_seq.unsqueeze(0), h0, c0) # out.shape: [1, n_classes, seq_len]; hn.shape: [1, 1, h_dim]
        ...
        for k in range(seq_len):
            ...
            out, hn, cn = model(next_input, hn, cn)
