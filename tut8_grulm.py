### Program implementing Neural Network Language Model (NNLM) using an GRU
...
## Todos / questions:
# 1. [solved] Do we need additional linear layer(s) after the GRU output? [fix: Yes, since we need to transform h_dim of output to n_classes]
# 2. [solved] add function in GRU to generate text given a single word or a sequence of words as input [fix: no need to add another function, just add a for loop over GRU forward prop with a input seq_len=1 ]
# 3. [solves] cross entropy loss for a batch of sequences: flattening the score vector from [seq_len, batch_size, n_classes] to [seq_len * batch_size, n_classes] gives wrong results. [fix: The documentation of CrossEntropyLoss says it can accept [batch_size, n_classes, seq_len]. Trying that works out]
...
class GRULM(nn.Module):
    def __init__(self, n_classes, embed_dim, h_dim):
        ...
        self.gru = nn.GRU(embed_dim, h_dim)
    ...
    def forward(self, inputs, h0): # inputs.shape: [batch_size, seq_len]; h0.shape: [1, batch_size, h_dim]
        ...
        out, hn = self.gru(x, h0) # out.shape: [seq_len, batch_size, h_dim]; hn.shape: [1, batch_size, h_dim]
...
if __name__ == '__main__':
    ...
    for ep in range(num_epochs):
        ...
        # calculate loss
        h0 = torch.zeros(1, batch_size, h_dim) # starting hidden state of GRU
    ...
    for i in range(seq_len, len(word_list)):
        ...
        h0 = torch.zeros(1, 1, h_dim) # starting hidden state of GRU
