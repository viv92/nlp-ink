### Program implementing Neural Network Language Model (NNLM) using an LSTM

## Features:
# 1. LSTM allows processing of input sequence of different lengths / num of words (though, for implementation, we need to fix on a max seq len)
...
class LSTMLM(nn.Module):
    ...
    def forward(self, inputs, h0, c0): # inputs.shape: [batch_size, seq_len]; h0.shape: [1, batch_size, h_dim]
        x = self.embedding(inputs) # x.shape: [batch_size, seq_len, embed_dim]
        x = x.transpose(0, 1) # x.shape: [seq_len, batch_size, embed_dim]
        out, (hn,cn) = self.lstm(x, (h0,c0)) # out.shape: [seq_len, batch_size, h_dim]; hn.shape: [1, batch_size, h_dim]
...
if __name__ == '__main__':
    ...
    for ep in range(num_epochs):
        ...
        # calculate loss
        h0 = torch.zeros(1, batch_size, h_dim) # starting hidden state of LSTM
        c0 = torch.zeros(1, batch_size, h_dim) # starting cell state of LSTM
    ...
    for i in range(seq_len, len(word_list)):
        ...
        h0 = torch.zeros(1, 1, h_dim) # starting hidden state of LSTM
        c0 = torch.zeros(1, 1, h_dim) # starting cell state of LSTM
