### Program implementing transformer from scratch in pytorch - inspired from @1 the Annotated Transformer and @2 graykode
...
# class to add positional encoding to embeddings (note that positional encoding is a constant untrainable vector)
...
def make_model(src_vocab_size, tgt_vocab_size, d_model, d_k, d_v, n_heads, n_layers, d_ff, dropout):
    ...
    posenc = PositionalEncoding(d_model, dropout) # positional encoding block
    ...
    decoder = Decoder(decoder_layer, n_layers) # decoder = stacked decoder layers
...
if __name__ == '__main__':
    ...
    # instantiate model
    model = make_model(len(src_vocab.keys()), len(tgt_vocab.keys()), d_model, d_k, d_v, n_heads, n_layers, d_ff, dropout).to(device)
