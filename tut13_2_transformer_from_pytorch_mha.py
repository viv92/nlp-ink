### Program implementing transformer from scratch in pytorch - version 3 (uses pytorch's internal nn.MultiheadAttention module for MultiHeadAttention)
...
# 2.1. Uses pytroch's inbuilt nn.MultiheadAttention for MultiHeadAttention, instead of a custom implementation
# 2.2. nn.MultiheadAttention expects separate padding and causal masks (so we don't combine them using boolean AND)
# 2.3. Change in expected mask shapes: padding_mask.shape = [batch_size, seq_len]; causal_mask.shape = [seq_len, seq_len]
# 2.4. True values in mask boolean are masked
...
def calculate_loss(model, minibatch, pad_token, device):
    ...
    dec_dec_mask_padding = pad_mask(dec_input, pad_token)
    enc_dec_mask = enc_enc_mask # since keys = enc_input for cross attention

    # create subsequent masking for dec_input - note that we don't combone the causal mask to padding mask (we want them separate when using nn.MultiheadAttention)
    dec_dec_mask_causal = subsequent_mask(dec_input.shape).to(device)

    # forward prop through model - note that we do not pass enc_dec_mask as its equal to enc_enc_mask
    scores = model(enc_input, enc_enc_mask, dec_input, dec_dec_mask_padding, dec_dec_mask_causal) # scores.shape: [batch_size, max_seq_len, tgt_vocab]
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask_padding):
        x = self.sublayers[0](x, lambda x: self.self_attn(x, x, x, mask_padding=mask_padding)) # x.shape: [batch_size, seq_len, d_model]
...
class EncoderDecoder(nn.Module):
    ...
    def encode(self, src, src_mask):
        ...
        return encoder_out
    def decode(self, tgt, encoder_out, src_mask, tgt_mask_padding, tgt_mask_causal):
        decoder_out = self.decoder(self.f_tgt_emb(tgt), encoder_out, src_mask, tgt_mask_padding, tgt_mask_causal)
        return decoder_out
    def forward(self, src, src_mask, tgt, tgt_mask_padding, tgt_mask_causal):
        encoder_out = self.encode(src, src_mask) # encoder_out.shape: [batch_size, seq_len, d_model]
        decoder_out = self.decode(tgt, encoder_out, src_mask, tgt_mask_padding, tgt_mask_causal) # decoder_out.shape: [batch_size, seq_len, d_model]
...
def greedy_decoding(encoder_out, enc_enc_mask, dec_input, max_seq_len, pad_token):
    for i in range(max_seq_len - 1):
        dec_dec_mask_padding = pad_mask(dec_input, pad_token)
        dec_dec_mask_causal = subsequent_mask(dec_input.shape).to(device)
        decoder_out = model.decode(dec_input, encoder_out, enc_enc_mask, dec_dec_mask_padding, dec_dec_mask_causal)
